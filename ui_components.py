...
def show_grade(has_grade, summary, semester_grade_list, free_credit):
    ...
    with tab3:
        ...
        edited_df = st.data_editor(has_grade.loc[:, ["Course", "Course Name", "Grade", "Grade_4", "Grade_10", "Credit"]], num_rows="dynamic")
        
        if st.button("Recalculate GPA", key="recalculate_overall"):
            if not edited_df.empty:
                map_grade_4 = {'A+': 4, 'A': 4, 'B+': 3.5, 'B': 3, 'C+': 2.5, 'C': 2, 'D+': 1.5, 'D': 1, 'F': 0}
                
                valid_grades = edited_df[edited_df["Grade"].isin(map_grade_4.keys())]
